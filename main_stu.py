...
#        self.login_btn.setStyleSheet(main_style.login_btn())
    ...
    def show_web(self):
        window = login_stu.WebView()
        window.resize(1440, 720)
        window.setMinimumWidth(800)
        window.setWindowTitle('登录')
        window.setWindowIcon(QIcon(window.get_resource_path(login_stu.icon_path + "ico.png")))
        window.show()
    ...
    def excel_batch_import(self):
        window = excelImport.ExcelReader()
        window.resize(1440, 720)
        window.setMinimumWidth(800)
        window.setWindowTitle('批量导入')
        window.show()
    ...
    def bind(self):
        ...
        self.choice_stu.activated.connect(lambda:self.controller.choice_stu(self.choice_stu.currentText()))
    ...
    def update(self,id,attribute,data):
        if not '?' in self.banji_input.text():
            self.banji_input.setStyleSheet(main_style.input_box())
        if not '职称' in self.zhichen_input.text():
            self.zhichen_input.setStyleSheet(main_style.input_box())
